...
import { createFeasibilityRouter } from '../routes/feasibilityRoutes.js';
import { createSessionHelpers } from './sessionMiddleware.js';
import { createStorageRouter } from '../routes/storageRoutes.js';

const BROWSER_CORE_MODULES = new Set([
...
export function createExpressApp({
  ...
  app.get('/modules/:category/:moduleName', (request, response, next) => {
    const { category, moduleName } = request.params;
    const modulePath = `${category}/${moduleName}`;
    if (!BROWSER_CORE_MODULES.has(modulePath)) return next();
    return response.sendFile(join(root, 'src', 'core', category, moduleName), (error) => {
      if (error) next(error);
    });
  });
