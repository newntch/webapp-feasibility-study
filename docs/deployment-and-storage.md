...
- `clinical_db`: owns the OMOP schema. The web role reads it but cannot mutate
  it. Cohort queries use bound SQL parameters.

Build and run the local stack with `docker compose build web` and
`docker compose up -d`. Check `/api/health` after restoring or verifying both
databases. `docker compose ps` shows
...
repository fixtures. Record the restored dataset provenance and do not replace
a clinical dataset without a validated refresh and cutover process.
