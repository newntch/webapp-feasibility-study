...
test('preserves the existing public page URLs without caching', async () => {
  await withTestServer({}, async (baseUrl) => {
    for (const pagePath of ['/', '/login.html', '/dictionary.html', '/logs.html']) {
      const response = await fetch(`${baseUrl}${pagePath}`);
      assert.equal(response.status, 200, `${pagePath} must remain available`);
...
test('serves only browser-required core modules from the categorized module URLs', async () => {
  await withTestServer({}, async (baseUrl) => {
    const publicModules = [
      '/modules/cohort/advancedConditions.js',
      '/modules/cohort/cohortEngine.js',
      '/modules/dictionary/masterDictionary.js',
      '/modules/filters/filterBuilderBehavior.js',
      '/modules/filters/filterBuilderDefaults.js',
      '/modules/sql/omopSqlBuilder.js',
      '/modules/sql/sqlBuilder.js'
    ];

    for (const modulePath of publicModules) {
      const response = await fetch(`${baseUrl}${modulePath}`);
      assert.equal(response.status, 200, `${modulePath} must remain browser-readable`);
...
test('does not expose server code or other repository files as static assets', async () => {
  await withTestServer({}, async (baseUrl) => {
    const forbiddenPaths = [
      ...
      '/src/cohortEngine.js',
      '/modules/dictionary/remoteDictionary.js',
      '/modules/server/config/config.js',
      '/modules/server/services/feasibilityService.js',
      '/config/app.config.example.json',
      '/tests/unit/server/serverConfig.test.mjs',
